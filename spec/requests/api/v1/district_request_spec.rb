...
describe 'district API' do
  ...
  it 'returns school district name, lea_id, enrollment, and financial info for a searched location' do
    ...
    expect(attributes[0]).to have_key(:district_name)
    expect(attributes[0]).to have_key(:urban_centric_locale)
    expect(attributes[0]).to have_key(:number_of_schools_in_district)
    expect(attributes[0]).to have_key(:enrollment)
    expect(attributes[0]).to have_key(:pre_k_teachers)
    expect(attributes[0]).to have_key(:kindergarten_teachers)
    expect(attributes[0]).to have_key(:elementary_teachers)
    expect(attributes[0]).to have_key(:secondary_teachers)
    expect(attributes[0]).to have_key(:total_teachers)
    expect(attributes[0]).to have_key(:student_teacher_ratio)
    expect(attributes[0]).to have_key(:instructional_aides)
    expect(attributes[0]).to have_key(:guidance_counselors)
    expect(attributes[0]).to have_key(:student_guidance_counselor_ratio)
    expect(attributes[0]).to have_key(:librarian_total)
    expect(attributes[0]).to have_key(:student_librarian_ratio)
    expect(attributes[0]).to have_key(:total_staff)
    expect(attributes[0]).to have_key(:district_expense_total)
    expect(attributes[0]).to have_key(:expenses_for_instruction)
    expect(attributes[0]).to have_key(:salaries_total)
    expect(attributes[0]).to have_key(:salaries_instruction)
    expect(attributes[0]).to have_key(:instruction_salary_percent_of_total)
    expect(attributes[0]).to have_key(:per_student_expenditure)
    expect(attributes[0]).to have_key(:per_teacher_salary_expenses)
    expect(attributes[0]).to have_key(:latitude)
    expect(attributes[0]).to have_key(:longitude)
  end
